...
const connection = new signalR.HubConnectionBuilder().withUrl("http://localhost:5005/chatHub").configureLogging(signalR.LogLevel.Information).build();
...
const appendFile = (user, fileName, fileData, messageClass) => {
    ...
    if (fileName.match(/\.(jpg|jpeg|png|gif)$/)) { // Check if the file is an image
